var map;
var lat = 40.77560182201292;
var lng = -73.95034790039062;
...
function get_container(loc_info_one_array) {
  ...
  $.each(loc_info_one_array, function(i, entry) {
    ret +=  
    '<div class="wrapper" data-link="' + entry.link + '"'
    + 'data-lat="' + entry.location.latitude + '" data-long="' + entry.location.longitude + '">'
    + '<div class="title"><div class="profile-picture">'
    + '<img src="' + entry.user.profile_picture + '"/></div><div class="from">' + entry.user.username + '</div></div><div class="photo">'
    + '<img src="' + entry.images.standard_resolution.url + '"/></div><div class="metadata">'
    + '<div class="caption">';
...
function addClickListener(map) {
  google.maps.event.addListener(map, 'click', function(e) {
    search_locations(map, e.latLng.lat(), e.latLng.lng());
  });
}

function search_locations(map, lat, lng) {
  loc_info = [];
  $.get(
